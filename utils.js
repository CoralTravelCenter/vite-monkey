...
export function observeElement(selector, onAppear, onDisappear) {
	let isPresent = false;
	...
	// Проверка на случай, если элемент уже есть на момент запуска
	const initialElement = document.querySelector(selector);
	if (initialElement) {
		isPresent = true;
		onAppear?.(initialElement);
	}

	return observer; // можно использовать для отключения через observer.disconnect()
}
