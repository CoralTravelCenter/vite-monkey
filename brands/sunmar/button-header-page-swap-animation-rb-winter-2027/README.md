# swap-button

Mindbox experiment based on Vite and vite-plugin-monkey.

## Settings

- Brand: `sunmar`
- Match: `https://www.sunmar.ru/*`
- Entry: `src/main.js`
- Styles: `src/style.css`

## Commands

```bash
npm run dev:experiment -- brands/sunmar/button-header-page-swap-animation-rb-winter-2027
npm run build:experiment -- brands/sunmar/button-header-page-swap-animation-rb-winter-2027
```

## Files

- `src/main.js` - entry point.
- `src/markup.html` - experiment markup.
- `src/style.css` - experiment styles.
- `experiment.config.json` - config for root-level `dev:experiment` and `build:experiment`.
